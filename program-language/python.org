...
#+begin_src emacs-lisp :outputs silent
  ;; 1) 进入 Python buffer 自动启动 Eglot
  (add-hook 'python-mode-hook #'eglot-ensure)
  ;; Emacs 29+ 若启用 tree-sitter，也顺手加上：
  ;; (with-eval-after-load 'python
  ;;   (when (boundp 'python-ts-mode)
  ;;     (add-hook 'python-ts-mode-hook #'eglot-ensure)))

  ;; 2) 指定 Python 用 jedi-language-server（而不是 pyright/pylsp）
  ;; (with-eval-after-load 'eglot
  ;;   (add-to-list 'eglot-server-programs
  ;;                '((python-mode python-ts-mode)
  ;;                  . ("jedi-language-server"
  ;;                     :initializationOptions
  ;;                     (:workspace (:extraPaths ["src"])
  ;;                      :completion (:resolveEagerly t)
  ;;                      :diagnostics (:enable t))))))

  ;; jedi-language-server 不支持
  ;; ;; （可选）保存时自动格式化（前提是服务端支持）
  ...
  ;; Optional: Ruff formatting command for Python buffers
  (defun ruff-format-buffer-v1 ()
  ...
  ;;           (lambda ()
  ;;             (local-set-key (kbd "C-c C-f") #'ruff-format-buffer-v1)))

  ;; (add-hook 'after-save-hook #'ruff-format-buffer)
#+end_src

#+RESULTS:
| (lambda nil (local-set-key (kbd C-c C-f) #'ruff-format-buffer-v4)) | (lambda nil (local-set-key (kbd C-c C-f) #'ruff-format-buffer)) | hs-minor-mode | eglot-ensure |
