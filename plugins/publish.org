  ...
  (defun f/sitemap-format-entry (entry style project)
    (let (format-entry
      ...
      (if (not (equal title nil))
          (concat entry-text title)
        )
      (if (not (equal subtitle nil))
          (concat entry-text subtitle)
        )
      (if (not (equal category nil))
          (concat entry-text category)
        )
      (if (not (equal abstract nil))
          (concat entry-text abstract)
        )
      (cond ((equal title "") (setq format-entry (format "nodisplay%s" file)))
            ((not (equal description nil)) (setq format-entry (format "[[file:%s][%s-|-%s]]" file title description)))
            (t (setq format-entry (format "[[file:%s][%s]]" file title)))
            )
      (message "%s" entry-text)
  ...
  (setq org-publish-project-alist
        '(
          ("blog"
           ...
           :html-preamble "<script src='https://blog.inmove.top/js/sitemap.js'></script>
  <link rel='stylesheet' type='text/css' href='https://blog.inmove.top/css/sitemap.css' />"
           ; :html-preamble "<a></a>"
           ; :html-postamble "<hr/>"
           )
          ("css"
